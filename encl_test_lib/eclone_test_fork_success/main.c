                        ...
                        struct enclave_run *run, uint64_t r8, uint64_t r9)
{
    ...
    if (run->exit_reason == EXIT_EEXIT)
    {
        ...
        case EEXIT_OCALL_CLONE:
            ...
            int ret = enclave_clone_bind((struct sgx_enclave_clone_metadata *)clone->metadata);
            if (ret)
            {
                ...
                int pid = fork();
                if (pid) {
                    // Parent process just try to run the enclave
                    ...
                    // Child process check whether the clone succeeded
                    // set ret == -1 if the clone failed
                    clone->ret = getpid();
                }
            }
