    ...
    if (run->exit_reason == EXIT_EEXIT)
    {
        ...
        case EEXIT_OCALL_CLONE:
            ...
            if (ret)
            {
                ...
                clone->ret = clone_ret;
                if (clone_ret) {
                    while (__atomic_load_n(&can_enter, __ATOMIC_SEQ_CST) != 1) { sleep(1); }
                }
            }
...
int main(void)
{
    ...
    */
    ...
    if (!pid) {
        ...
        close(exitfd[0]);
        if (isPipeClosed(exitfd)) {
            sleep(1);
            // make sure parent exit here, let child enter then
            __atomic_store_n(&can_enter, 1, __ATOMIC_SEQ_CST);    
        }
    }
