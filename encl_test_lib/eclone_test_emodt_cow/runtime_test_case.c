...
int runtime_test_case() {
    ...
    if (!child) {
        // Try to convert a page to PT_TRIM
        ...
        __atomic_store_n(&state, 1, __ATOMIC_SEQ_CST);
    } else {
        do {} while (__atomic_load_n(&state, __ATOMIC_SEQ_CST) != 1);
        // try to write at the parent trim page
        *(char *)page = 'a';
        char write_page_success[] = "Child writes on the page after parent accept emodt trim page with the same vaddr!\n";
